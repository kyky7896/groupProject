	...
	<div id="wrap">
		...
		<section id="container">
			...
			<div id="content" class="ev_content">
				...
				<div class="ev_wrap">
					<h1 class="title" align="center">주문상세보기/수정</h1><br>
					...
					<h2>주문번호: <%=ob.getF_or_trade_num() %></h2><br>
					...
					%>
					  ...
					  <tr>
						<td><%=orderbean.getF_or_gs_name() %></td>
				...
				<%-- 	<br><h3>배송지정보</h3><br>
						...
						전화번호 : <%=ob.getF_or_rec_phone() %><br>
						배송지 우편번호 : <%=ob.getF_or_rec_postcode() %><br>
						배송지 주소 : <%=ob.getF_or_rec_address() %><br>
						배송지 상세 주소 : <%=ob.getF_or_rec_detailAddress() %><br>
						배송지 참조 주소 : <%=ob.getF_or_rec_extraAddress() %><br>
						기타 요구사항 : <%=ob.getF_or_memo() %><br> --%>
						<!-- 테이블 형태로 변경함... 혹시 기능이 안먹히면 주석된 자료 참고하여 추가하면 됩니다. -백성진- -->
				
			<table border="1" style="width: 1000px; vertical-align: middle;">	
				<tr style="background-color: #fff7e6;">
				  <td colspan="2"><b>배송지정보</b></td>
				  
				</tr>	
				<tr>
				  <td class="ob_getF_or">받는사람 :</td>
				  <td><%=ob.getF_or_rec_name() %></td>
				</tr>	
				<tr>
				  <td class="ob_getF_or">전화번호 :</td>
				  <td><%=ob.getF_or_rec_phone() %></td>
				</tr>
				<tr>
				  <td class="ob_getF_or">배송지 우편번호 :</td>
				  <td><%=ob.getF_or_rec_postcode() %></td>
				</tr>
				<tr>
				  <td class="ob_getF_or">배송지 주소 :</td>
				  <td><%=ob.getF_or_rec_address() %></td>
				</tr>
				<tr>
				  <td class="ob_getF_or">배송지 상세 주소 :</td>
				  <td><%=ob.getF_or_rec_detailAddress() %></td>
				</tr>
				<tr>
				  <td class="ob_getF_or">배송지 참조 주소 :</td>
				  <td><%=ob.getF_or_rec_extraAddress() %></td>
				</tr>
				<tr>
				  <td class="ob_getF_or">기타 요구사항 :</td>
				  <td><%=ob.getF_or_memo() %></td>
				</tr>	
			</table><hr>
			...
			<table border="1" >	
				...
				<tr>
				  <td>입금자명 :</td>
				  <td><%=ob.getF_or_trade_payer() %></td>
				</tr>
				<tr>
				  <td>주문상태 :</td>
				  <td><%=ob.getF_or_status() %>&nbsp;&nbsp;&nbsp;
				  <select name="status">
						<option value="0" <% if(ob.getF_or_status()==0){%>selected<%} %>>대기중</option>
						<option value="1" <% if(ob.getF_or_status()==1){%>selected<%} %>>발송준비</option>
						<option value="2" <% if(ob.getF_or_status()==2){%>selected<%} %>>발송완료</option>
						<option value="3" <% if(ob.getF_or_status()==3){%>selected<%} %>>배송중</option>
						<option value="4" <% if(ob.getF_or_status()==4){%>selected<%} %>>배송완료</option>
						<option value="5" <% if(ob.getF_or_status()==5){%>selected<%} %>>주문취소</option>
					</select>
					</td>
				</tr>
				<tr>
				  <td>운송장번호:</td>
				  <td>
				  <input type="text" name="trans_num" value="<%=ob.getF_or_trans_num()%>">
					</td>
				</tr>
				<tr>
					<td colspan="2"><input type="submit" style="width: 100px; height: 35px; background-color: #e0ebeb;" value="수정"></td>
				  </tr>	
			</table>
			</form>				
					<%-- <h3>결제정보</h3>
					...
					운송장번호:
						<input type="text" name="trans_num" value="<%=ob.getF_or_trans_num()%>"><br><hr><br>
						<input type="submit" style="width: 70px; height: 35px;" value="수정">
					
					
					</form> --%>
					<!-- 마찬가지로 디자인 깔끔함을 위해서 테이블 형태로 변경합니다.  기존자료 주석처리함. -백성진-  -->
